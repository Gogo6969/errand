...
async fn a_thread_reopened_tomorrow_is_the_same_conversation() {
    ...
    let (mut first, events) = Claude::open(&id, &home, false).expect("starting a thread");
    first
    ...
    let (mut again, events) = Claude::open(&id, &home, true).expect("reopening the thread");
    again
...
async fn reopening_something_that_was_never_there_says_so_rather_than_hanging() {
    ...
    let (_it, events) = Claude::open(never, &home, true).expect("spawning at all");
