// Lifted from KinAI and deliberately left in its own house style. The lints
// below are ours, not its, and quietening them here rather than editing the
// file keeps this a copy: the day KinAI learns something new about a backend,
// re-syncing should be a `cp` and not a merge.
...
const LLM_PORT_SINGLES: &[LlmPort] = &[
    LlmPort {
        port: 80,
        provider: "openai-compat",
        label: "OpenAI-compat (proxy)",
    },
    LlmPort {
        port: 443,
        provider: "openai-compat",
        label: "OpenAI-compat (TLS)",
    },
    LlmPort {
        port: 8443,
        provider: "openai-compat",
...
async fn caps_via_ollama(client: &reqwest::Client, base: &str, model: &str) -> Result<ModelCaps> {
