...
pub use super::stream::{ChatDelta, StreamHandle};
...
impl LlmClient {
    ...
    pub async fn list_models(&self) -> Result<Vec<String>> {
        super::find::list_models(&self.settings).await
    }
        ...
        ChatMessage::User {
            ...
            let content_value = if image_data_urls.is_empty() {
                ...
                let mut parts: Vec<serde_json::Value> =
                    Vec::with_capacity(image_data_urls.len() + 1);
                if !content.is_empty() {
...
mod tool_choice_tests {
    ...
    fn unrelated_errors_are_left_alone() {
        assert!(!is_tool_choice_rejection(
            "LLM error 401 Unauthorized: bad api key"
        ));
